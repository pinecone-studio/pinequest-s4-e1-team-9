  ...
  useEffect(() => {
    ...
    const initializeWebGL = () => {
      ...
      const renderer = new Renderer({
        dpr: Math.min(window.devicePixelRatio, DPR_CAP) * RENDER_SCALE,
        alpha: true,
      });
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updateSize = () => {
        ...
        renderer.dpr =
          Math.min(window.devicePixelRatio, DPR_CAP) * RENDER_SCALE;
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full h-full overflow-hidden pointer-events-none [contain:strict] [transform:translateZ(0)] ${className}`.trim()}
    />
  );
};
