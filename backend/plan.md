...
- [ ] **Refine `features/documents/ingest.service.ts`:** Ensure the PDF ingestion service correctly chunks content (800 token size, 150 overlap) and stores both chunks and embeddings in the appropriate Supabase tables.
...
- [ ] **Implement Context Injection:** Update `features/chat/chat.service.ts` to take the user query, perform a vector search, retrieve the top 5 chunks, and inject them as context into the LLM prompt.
...
2. **Retrieval Integration:** Keep `features/retrieval/vector-store.ts` connected to `features/chat/chat.service.ts`.
